...
int main(int argc, const char *argv[]){
	...
	if(param.method == Parameters<long long, double>::VB){
		...
		InferenceData<long long, double> maxAns = inference.vbFull(param.isDBUpdate, 1e-5, param.pathOutputDir+"/err");
		for(int i = 0; i < 10-1; i++){
		    InferenceData<long long, double> ans = inference.vbFull(param.isDBUpdate, 1e-5, param.pathOutputDir+"/err");
